...
// record of what shipped) and index.html's ROADMAP array (structured status/items/notes
...
const doc = readFileSync(path.join(ROOT, 'index.html'), 'utf8');
...
if (missingFromRoadmap.length === 0 && missingFromChangelog.length === 0) {
  console.log('check-roadmap-sync: changelog.md and index.html ROADMAP phase numbers match.');
  process.exit(0);
}

if (missingFromRoadmap.length) {
  console.warn(`check-roadmap-sync: changelog.md mentions Phase(s) ${missingFromRoadmap.join(', ')} with no matching card in index.html's ROADMAP.`);
}
if (missingFromChangelog.length) {
  console.warn(`check-roadmap-sync: index.html's ROADMAP has Phase(s) ${missingFromChangelog.join(', ')} never mentioned in changelog.md.`);
}
